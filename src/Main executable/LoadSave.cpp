...
void Nation::CloseNation()
{
	...
	{
		if (History[i]) // Проверка на nullptr
		{
			delete[] History[i]; // Используем delete[] вместо free
			History[i] = nullptr; // Устанавливаем в nullptr для безопасности
		}
	}
	if (History) // Освобождаем массив указателей
	{
		delete[] History; // Предполагается, что History выделен через new char*[]
		History = nullptr;
	}
	NHistory = 0;
	NMon = 0;
...
{
	if (NInside) {
		delete[] Inside; // Используем delete[] вместо free для корректного освобождения массива
		Inside = nullptr;
	}
	if (PathX) {
		delete[] PathX; // Используем delete[] вместо free для массива PathX
		PathX = nullptr;
	}
	if (PathY) {
		delete[] PathY; // Используем delete[] вместо free для массива PathY
		PathY = nullptr;
	}
	CPdestX = 0;
	CPdestY = 0;
	...
	if (IFire) {
		IFire->Erase();
		free(IFire); // Здесь IFire предполагается выделенным через malloc, оставляем free
		IFire = nullptr;
	}
}
void InitPulse();
